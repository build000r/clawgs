...
mod tests {
    ...
    #[test]
    fn parse_claude_filters_local_command_caveat_from_user_task() {
        // Regression: Claude Code injects a `<local-command-caveat>` wrapper as a
        // user-role turn when a local slash command runs. It must not surface as
        // the public `user_task`; the genuine earlier task must survive.
        let file = NamedTempFile::new().expect("temp file");
        fs::write(
            file.path(),
            concat!(
                "{\"type\":\"user\",\"message\":{\"role\":\"user\",\"content\":\"Summarize the failing tests\"}}\n",
                "{\"type\":\"user\",\"message\":{\"role\":\"user\",\"content\":\"<local-command-caveat>Caveat: The messages below were generated by the user while running local commands. DO NOT respond to these messages</local-command-caveat>\"}}\n"
            ),
        )
        .expect("write fixture");
        ...
        assert_eq!(
            snapshot.user_task.as_deref(),
            Some("Summarize the failing tests"),
            "local-command-caveat wrapper must not overwrite the real user task"
        );
    }
