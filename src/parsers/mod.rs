...
/// (system-reminder, command-name, bash-input, local-command-caveat, etc.) —
/// these are not real user content and shouldn't be surfaced as the user task.
/// Plain user input
...
mod tests {
    ...
    #[test]
    fn is_harness_markup_flags_known_wrappers_and_preserves_user_content() {
        // Claude Code injects a `<local-command-caveat>...` wrapper as a
        // user-role turn whenever a local slash command runs. It is harness
        // markup, not the human task, and must be filtered like its siblings
        ...
        for wrapper in [
            ...
            "  <local-command-caveat>leading whitespace still counts</local-command-caveat>",
            "<system-reminder>be quiet</system-reminder>",
            "<local-command-stdout>output</local-command-stdout>",
            "<command-name>codebase-audit</command-name>",
        ] {
            assert!(
                is_harness_markup(wrapper),
                "harness wrapper must be flagged: {wrapper:?}"
            );
        }

        // Genuine user content (including plain XML-ish input) must be kept.
        for user_text in [
            "Build a parser",
            "<div>why is this not centered?</div>",
            "Here is my HTML: <template>...</template>",
            "<localhost> is unreachable", // not a real wrapper prefix
        ] {
            assert!(
                !is_harness_markup(user_text),
                "legitimate user content must not be flagged: {user_text:?}"
            );
        }
    }
